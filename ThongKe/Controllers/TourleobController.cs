    ...
    public class TourleobController : BaseController
    {
        ...
        public async Task<IActionResult> GetKhachvetoursExcel(string tungay = null, string denngay = null, string chinhanh = null, string quay = null)
        {
            ...
            var d = (from p in list.VetourDTOs
                      group p by new { p.TuyenTq} into g
                      select new VetourDTOgroupTuyentqChinhanh
                      {
                          TuyenTq = g.Key.TuyenTq,
                          Sgtcode = string.Join(",", g.Select(x => x.Sgtcode).Distinct()),
                          VetourDTOs = g.ToList()
                      }).OrderBy(x => x.TuyenTq).ToList();
            string fromTo = "";
            ...
            xlSheet.Column(2).Width = 30;//Tuyến tq
            xlSheet.Column(3).Width = 10;//Số khách
            xlSheet.Column(4).Width = 20;//Doanh số
            //xlSheet.Column(5).Width = 60;//Sgtcode
            ...
            Tool.setCenterAligment(5, 1, 5, 5, xlSheet);
            //du lieu
            int iRowIndex = 6;
            int idem = 1;
            ...
            if (d != null && d.Count > 0)
            {
                foreach (var vm in d)
                {
                    ...
                    xlSheet.Cells[iRowIndex, 1].Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                    xlSheet.Cells[iRowIndex, 2].Value = vm.TuyenTq;
                    xlSheet.Cells[iRowIndex, 3].Value = vm.VetourDTOs.Sum(x => x.Chiemcho);
                    xlSheet.Cells[iRowIndex, 4].Value = vm.VetourDTOs.Sum(x => x.DoanhThu);
                    //xlSheet.Cells[iRowIndex, 5].Value = vm.Sgtcode;
                    //xlSheet.Cells[iRowIndex, 5].Style.WrapText = true;
                    //xlSheet.Cells[iRowIndex, 5].Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                    xlSheet.Cells[iRowIndex, 1, iRowIndex, 5].Style.Font.SetFromFont(new Font("Times New Roman", 12, FontStyle.Bold));
                    
                    iRowIndex++;
                    var d1 = (from p in vm.VetourDTOs
                              group p by new { p.Chinhanh } into g
                              select new VetourDTOgroupTuyentqChinhanh
                              {
                    ...
                    {
                        xlSheet.Cells[iRowIndex, 2].Value = item.Chinhanh; // chinhanh xuat ve
                        xlSheet.Cells[iRowIndex, 3].Value = item.VetourDTOs.Sum(x => x.Chiemcho); // theo tuyentq, chinhanh xuat ve
                        xlSheet.Cells[iRowIndex, 4].Value = item.VetourDTOs.Sum(x => x.DoanhThu); // theo tuyentq, chinhanh xuat ve
                        
                        iRowIndex ++;
                    }
                    var count = (d1.Count + i); // (d1.Count == 1 ? i : (d1.Count + i));
                    //var count = (d1.Count == 1 ? i : (d1.Count - 1 + i));
                    xlSheet.Cells[$"A{i}:A{count}"].Merge = true;
                    xlSheet.Cells[i + 1, 1, iRowIndex, 5].Style.Font.SetFromFont(new Font("Times New Roman", 10, FontStyle.Regular));
            ...
            xlSheet.Cells[iRowIndex, 3].Value = list.VetourDTOs.Sum(x => x.Chiemcho); // theo tuyentq, chinhanh xuat ve
            xlSheet.Cells[iRowIndex, 4].Value = list.VetourDTOs.Sum(x => x.DoanhThu); // theo tuyentq, chinhanh xuat ve
            xlSheet.Cells[iRowIndex, 1, iRowIndex, 5].Style.Font.SetFromFont(new Font("Times New Roman", 12, FontStyle.Bold));
            Tool.setBorder(5, 1, iRowIndex, 4, xlSheet);
            ...
            Tool.NumberFormat(6, 3, iRowIndex, 4, xlSheet);
